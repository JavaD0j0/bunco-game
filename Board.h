...
namespace cs31
{
    ...
    public:
		...
		//MUTATORS
        void setCurrentRound( int round );    // set the current round
        void markHumanAsWinner();             // set human as the round winner
        void markComputerAsWinner();          // set the computer as the round winner
        
		//ACCESSORS
        int countUpComputerRoundWins( ) const;// how many rounds has the computer won so far?   
        int countUpHumanRoundWins( ) const;   // how many rounds has the comptuer won so far?
        std::string display( ) const;         // stringify this Board
    private:
